...
int main(int argc, char **argv)
{
    ...
    Eigen::Matrix<double, 6, 1> i_tracking_err;
    i_tracking_err.setZero();
    ...
    if (!public_node_handler.getParam("i_gains", i_gains_) || i_gains_.size() != 6) {
    ROS_ERROR(
        "JointImpedanceExampleController:  Invalid or no k_gain parameters provided, aborting "
        "controller init!");
    return false;
    ...
    ki(4,0)= i_gains_[4];
    ki(5,0)= i_gains_[5];
    ROS_INFO("I'm sending way point");
    while (ros::ok())
    {
        ...
        {   
            ...
            if(counter_traj < (target_pose.poses.size())){
                ...
                Eigen::Matrix<double, 6, 1> i_tracking_err = tracking_err*0.001;
                Eigen::Matrix<double, 6, 6> Kp = kp.asDiagonal();
                Eigen::Matrix<double, 6, 6> Kd = kd.asDiagonal();
                Eigen::Matrix<double, 6, 6> Ki = ki.asDiagonal();           
                Eigen::Matrix<double, 6, 1> force_cmd = (Kp * tracking_err) - Kd * twistEETf_filt + (Ki * i_tracking_err) ;
