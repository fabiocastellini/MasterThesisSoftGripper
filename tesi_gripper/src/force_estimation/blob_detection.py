...
# Function to detect markers. User can decide to detect circles or ellipses (not sure there are benefits/it works as expected)
def detect_markers(frame, marker_shape='circle'):
    ...
    if marker_shape == 'circle':
        if show_prints:
            print("[1 - Online tracking] Detected", num_detected_markers, "circular markers!")
    elif marker_shape == 'ellipse':
        if show_prints:
            print("[1 - Online tracking] Detected", num_detected_markers, "elliptic markers!")

    return frame, markers, num_detected_markers


# Function to check if point is inside a circle, to filter out markers detected on borders
